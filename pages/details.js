...
class RecordDetails extends Component {
  static async getInitialProps(props) {
    ...
    // Initializing variables to store records, appointment, and profile picture
    var records, records2, appointment, profilePic;

    try {
      // Fetching patient demographic data
      records = await record.methods
        .searchPatientDemographic(addr)
        .call({ from: accounts[0] });

      // Fetching patient medical data
      ...
      // Checking if the appointment address is empty and setting it to an empty string if true
      if (appointment[0].includes("0x00000000000")) appointment[0] = "";

      // Setting the profile picture based on the patient's gender
      profilePic =
        records[3] == "Male"
  ...
  // Method to render the display of the component
  renderDisplay() {
    return (
